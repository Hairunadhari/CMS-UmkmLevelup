    ...
    <div v-if="field">
      <div class="flex">
        <h2 class="text-2xl font-semibold z-10 truncate mb-5 text-gray-900 flex-grow">
          Ubah "<span class="truncate">{{ field.name }}</span>" pilihan
        </h2>
        <div class="flex justify-center mb-2 mr-5">
          <div>
            <v-button color="light-gray" size="small" @click="removeBlock">
              <svg class="h-4 w-4 text-red-600 inline mr-1 ml-1 -mt-1" viewBox="0 0 24 24" fill="none"
          ...
          <div class="ml-1">
            <v-button size="small" color="light-gray" @click="duplicateBlock">
              <svg class="h-4 w-4 text-blue-600 inline mr-1 ml-1 -mt-1" viewBox="0 0 24 24" fill="none"
                   xmlns="http://www.w3.org/2000/svg">
                <path
          ...
          <change-field-type class="my-1 hidden" :field="field" @changeType="onChangeType"/>
        </div>
      </div>
      ...
      <div class="-mx-4 sm:-mx-6 p-5 border-b border-t">
        ...
        <p class="text-gray-400 mb-5">
          Sembunyikan pilihan ini dan/atau wajib isi
        </p>
      ...
      <div v-if="field.type === 'checkbox'" class="-mx-4 sm:-mx-6 p-5 border-b">
        ...
        >
          Aktifkan Hanya pilih satu
        </v-checkbox>
        <p class="text-gray-400 mb-5">
          Jika Aktif, Checklist akan memilih satu saja
        </p>
      </div>
      ...
      <div v-if="field.type === 'number'" class="-mx-4 sm:-mx-6 p-5 border-b">
        ...
        <p class="text-gray-400 mb-5">
          Jika Aktif akan muncul rating pada pilihan opsi nomor
        </p>
      ...
      <div v-if="field.type === 'date'" class="-mx-4 sm:-mx-6 p-5 border-b">
        ...
        >
          Non aktifkan tanggal sebelumnya
        </v-checkbox>
        ...
        >
          Non aktifkan tanggal selanjutnya
        </v-checkbox>
      </div>
      ...
      <div v-if="['select','multi_select'].includes(field.type)" class="-mx-4 sm:-mx-6 p-5 border-b">
        ...
        >
          Aktifkan untuk dapat membuat pilihan baru
        </v-checkbox>
        <v-checkbox v-model="field.without_dropdown" class="mt-4 hidden"
      ...
      <div v-if="displayBasedOnAdvanced" class="-mx-4 sm:-mx-6 p-5 border-b">
        ...
        <p class="text-gray-400 mb-5">
          Ubah Inputan nama, Isian default, Info.
        </p>
        ...
        />
        <div v-if="['select','multi_select'].includes(field.type)" class="-mt-3 mb-3 text-gray-400 dark:text-gray-500 hidden">
          <small>
        ...
        <text-input v-if="hasPlaceholder" name="placeholder" class="mt-4"
                    :form="field"
                    label="Isian ketika kosong (Placeholder)"
        />
        ...
        <text-input name="help" class="mt-4"
                    ...
                    help="keterangan ini akan muncul dibawah Inputan yang dituju."
        />

        <select-input name="width" class="mt-4"
                      :options="[
                        {name:'Penuh (Lebar = 100%)',value:'full'},
                        {name:'1/2 (Lebar = 50% )',value:'1/2'},
                        {name:'1/3 (Lebar = 33%)',value:'1/3'},
                        {name:'2/3 (Lebar = 66%)',value:'2/3'},
                        {name:'1/4 (Lebar = 25%)',value:'1/4'},
                        {name:'3/4 (Lebar = 75%)',value:'3/4'},
                      ]"
                      :form="field" label="Lebar Inputan"
        />
        ...
        <template v-if="['text','number','url','email','phone_number'].includes(field.type)">
          <text-input v-model="field.max_char_limit" name="max_char_limit" native-type="number" :min="1" :max="2000"
                      :form="field"
                      label="Maksimal Inputan"
                      help=""
                      :required="false"
          />
          <checkbox-input name="show_char_limit" :form="field" class="mt-4"
                          label="Aktifkan untuk memeperlihatkan maksimal input"
          />
        </template>
      ...
      <!--  Advanced Options   -->
      <div v-if="field.type === 'text'" class="-mx-4 sm:-mx-6 p-5 border-b hidden">
        <h3 class="font-semibold block text-lg">
          Advanced Options
          <pro-tag/>
        </h3>
