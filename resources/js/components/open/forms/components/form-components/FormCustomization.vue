<template>
  <collapse class="p-4 w-full border-b" :default-value="isCollapseOpen" @click="onClickCollapse">
    ...
    <select-input name="theme" class="mt-4 hidden"
                  :options="[
    ...
    <select-input name="dark_mode" class="mt-4 hidden"
                  ...
                  :form="form" label="Mode Gelap"
    />
    <color-input name="color" class="mt-4"
                 :form="form"
                 label="Warna (untuk tombol & garis luar input)"
    />
    <toggle-switch-input name="hide_title" :form="form" class="mt-4"
                    label="Sembunyikan Judul"
    />
    <toggle-switch-input name="no_branding" :form="form" class="mt-4 hidden"
                    label="Remove Form Builder Branding"
    />
    <toggle-switch-input name="uppercase_labels" :form="form" class="mt-4"
                    label="Huruf Besar Semua Judul Input"
    />
    <toggle-switch-input name="transparent_background" :form="form" class="mt-4"
                    label="Background Transparan" help="Akan terlihat ketika di embed / dijalankan"
    />
  </collapse>
</template>
